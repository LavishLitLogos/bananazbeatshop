...
export function CreditsRoom() {
  ...
  const handleShare = async (song?: CreditSong) => {
    ...
    const title = song ? `${song.title} · Produced by ThisBeatIzBananaz` : 'ThisBeatIzBananaz Credits';
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title,
          text: song
            ? `Listen to "${song.title}" by ${song.artist_name || 'Artist TBA'} — produced by ThisBeatIzBananaz.`
