...
export function TheLabRoom() {
  ...
  const sendMessage = async () => {
    if (!isAdmin) {
      addToast('The Lab is announcement/react mode right now.', 'info');
      return;
    }
    ...
    if (error) {
      ...
      addToast('Lab announcement posted.', 'success');
    }
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 pb-40">
        <div className="rounded-2xl border border-[#1d1d1d] bg-[#101010] p-4">
          ...
          <p className="mt-2 text-xs text-[#777] leading-relaxed">
            The Lab is in announcement/react mode. Admin posts updates, drops, co-lab notes,
            and alerts. FAMZ can react without clogging the room.
          </p>
        </div>
        ...
        {loading ? (
          ...
          messages.map((message) => (
            <MessageBubble
              ...
              currentUser={currentDisplayName}
              isAdmin={isAdmin}
        ...
        {isAdmin && announcementMode && (
          <div className="px-4 pt-2">
            <div className="rounded-xl border border-[#f5c518]/25 bg-[#f5c518]/10 px-3 py-2 text-[11px] text-[#f5c518]">
              Announcement mode active. This post is from admin and can include @ALL.
            </div>
          </div>
        )}
          ...
          <div className="flex flex-col items-center gap-2 px-4 py-3">
            <div className="flex items-center gap-2 w-full">
              <input
                ...
                placeholder={`Admin announcement in #${activeTopic}...`}
                value={text}
          ...
          <div className="px-4 py-3 text-center">
            <div className="rounded-2xl bg-[#111] border border-[#1e1e1e] px-4 py-3">
              ...
              <div className="text-[11px] text-[#666] mt-1">
                Tap a post, hit React, and show love without flooding the room.
              </div>
            </div>
          </div>
        )}
